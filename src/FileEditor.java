...
public class FileEditor {
    ...
    public String readData(String prefix, String splitter) { // этот метод считывает дату по переданным значениям предназначен для класса Session
        return read(prefix, splitter);
    }

    public String readCheque(int chequeNumber) { // этот метод считывает чек из файла Check по заданному номеру чека предназначен для класса Session
    ...
    public void writeData(String data, String prefix) { // этот метод записывает файлы по заданным параметрам предназначен для класса Session
        boolean isCheque = true;
        try {
            if (prefix != EnumFileTools.CHEQUE_INDEX.getTool()) {
                isCheque = false;
    ...
    public void writeMap(Map<Integer, Character[]> map) { // этот метод записывает карту в файл FullInfo предназначен для класса HallMap
        try {
            this.out = new FileWriter(fileAllData, true);
            this.writer = new BufferedWriter(out);
